...
class View
{
	...
	*/
	function RenderBody()
	{
		...
		if(file_exists($fileview)){
			...
			/**
			* not view exists ? redirect 404
			*/
			ErrorHandler::ViewNotFound($fileview);
		}
	}
	...
	*/
	public  function Action($Controller,$Action,array $data=null)
	{

		$ControllerName = $Controller . Config::get('CONTROLLER_PREFIX');
		$ActionName = $Action . Config::get('ACTION_PREFIX');
		
		self::LoadController(Config::get('CONTROLLER_PATH'),$ControllerName . Config::get('CONTROLLER_FILE_EXTENSION'));
	...
	function LoadController($location,$controllerfile) 
	{    
